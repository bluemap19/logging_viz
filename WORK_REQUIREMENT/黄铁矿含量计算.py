...
if __name__ == '__main__':
    ...
    data_logging = data_well.get_logging(key='Z:\\logging_workspace\\姬119H2\\姬119H2岩扫_logging_data.csv')
    # 常规测井无效数据归
    ...
    # 图像二值化，先开后闭使目标区域数量更多
    result = cal_fmis_segmentation(imgs=[target_fmi.copy()], depth=target_depth, windows=400, step=200, post_method=['open', 'close', ], method_configs={
    ...
    total_pixels, target_mask = count_target_areas(result[list(result.keys())[0]][0], target_config={
        'area_range': (1, 120),
        'width_range': (0, 20),
        'height_range': (0, 20),
    ...
    result_pyrite = cal_fmis_pyrite_content(
            ...
            config_windows = {'window_length': 300, 'step': 5, 'depth_col_name': 'DEPTH', 'curves_names_list': [target_method+'_fmi']},
    )
